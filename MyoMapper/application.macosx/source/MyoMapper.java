...
public class MyoMapper extends PApplet {
...
public void setup() {
  ...
  windowX = PApplet.parseInt(displayWidth/1.5f);
  windowY = displayHeight-PApplet.parseInt(displayHeight/10);
...
public void labels(){
  ...
  text("MyoMapper", width/2+width/8,height/18);
   ...
   text("Balandino Di Donato - 2015", width-cons.width/1.15f, height-height/40); 
   text("www.balandinodidonato.com", width-cons.width/1.15f, height-height/120);
...
public void logoSetup(){
  ...
  int resizeConsFactor = PApplet.parseInt(width/160);
  int resizeLabFactor = PApplet.parseInt(width/200);
  cons.resize(PApplet.parseInt(cons.width/resizeConsFactor),PApplet.parseInt(cons.height/resizeConsFactor));
  integralogo.resize(PApplet.parseInt(integralogo.width/resizeLabFactor),PApplet.parseInt(integralogo.height/resizeLabFactor));
...
public void myoOnEmg(Device myo, long timestamp, int[] data) {
  ...
  for(int i=0; i<8; i++){
    sensors.get(i).add(PApplet.parseInt(abs(emg[i])*(-0.3125f)+PApplet.parseInt(windowX/17)));
    ...
    for(int i=0; i<17; i++){
      if(!sensors.get(i).isEmpty()){
        ...
        for(int j=100; j<sensors.get(i).size(); j++){
          vertex(j, sensors.get(i).get(j)+(i*(PApplet.parseInt(windowY/17))));
        }
        endShape();
      } 
    }
...
public void MIDI_CH(int value) { 
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "MyoMapper" };
    if (passedArgs != null) {
